...
export const updateSeatsStatus = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const {  row, seatNumber, status } = req.query;

        const updatedShow = await ShowService.updateSeatsStatus(req.params.showId as string,
